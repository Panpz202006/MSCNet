...
from utils.tools import get_logger,set_seed,get_optimizer,get_scheduler,save_images,calculate_params_flops
import torch
...
def main():
    ...
    train_datasets_path=os.path.join(cwd_path,'data','PH2','Datasets')
    val_datasets_path=os.path.join(cwd_path,'data','PH2','Datasets')
    ...
    num_workers=0
    image_size=(224,224)
    train_dataset=PH2_Datasets(train_datasets_path,Train_Transformer(image_size))
    ...
    val_dataset=PH2_Datasets(val_datasets_path,Train_Transformer(image_size),val=True)
    ...
    is_test=False
    if is_test:
        test_epoch(model=model,save_path=save_path,test_loader=val_loader,criterion=criterion,logger=logger)
        input('输入回车：继续')
    print('####################开始训练####################')
