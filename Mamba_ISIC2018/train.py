...
def train_epoch(train_loader,model,criterion,optimizer,epoch, steps,logger,save_cycles=5):
    ...
    for step,data in enumerate(train_loader):
        ...
        if step%save_cycles==0:
            ...
            log_info=f'train: epoch={epoch}, step={step}, loss={np.mean(loss_list):.4f}, lr={lr:.4f},loss1={loss1.item():.4f},loss2={loss2.item():.4f},loss3={loss3.item():.4f},loss4={loss4.item():.4f}'
            print(log_info)
            logger.info(log_info)
    adjust_lr(optimizer, epoch, init_lr=1e-3, decay_rate=0.5, decay_epoch=50)
